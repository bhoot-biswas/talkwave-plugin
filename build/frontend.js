...
function formatTime(totalSeconds) {
  ...
  return hours > 0 ? `${padNum(hours)}:${padNum(minutes)}:${padNum(seconds)}` : `${padNum(minutes)}:${padNum(seconds)}`;
}
const {
  state,
  actions
} = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.store)("talkwave", {
  state: {
    playlists: {},
    currentPlaylistId: null,
    // updated later in play()
    index: 0,
    duration: 0,
    seek: 0,
    loading: false,
    ...
    get isPlaying() {
      ...
      return state.currentPlaylistId === playlist_id && state.index === index && state.playing;
    },
    get isLoading() {
      const {
        index,
      ...
      return state.currentPlaylistId === playlist_id && state.index === index && state.loading;
    },
    get playlistLoading() {
      const {
        playlist_id
  ...
  actions: {
    play: (index = 0, playlistId = null) => {
      ...
      if (track.howl) {
        ...
        sound = track.howl = new howler__WEBPACK_IMPORTED_MODULE_1__.Howl({
      ...
      if (state.playing) {
        let currentSound = state.playlists[state.currentPlaylistId][state.index].howl;
    ...
    volume: val => {
      howler__WEBPACK_IMPORTED_MODULE_1__.Howler.volume(val);
    },
    seek: per => {
      let sound = state.playlists[state.currentPlaylistId][state.index].howl;
    ...
    handleRate: () => {
      let sound = state.playlists[state.currentPlaylistId][state.index].howl;
      let newRate = state.rate < 2 ? (parseFloat(state.rate) + 0.2).toFixed(1) : 0.4;
      state.rate = newRate;
      sound.rate(newRate);
    },
    toggleMute: () => {
